...
void CImgWnd::ShowDownSampleImg24(int width, int height, int widthstep, BYTE* pData, int nSampleRate) {
	m_dibShow.CreateDIB(width/2, -height/2, 24);
	if( width%nSampleRate || height%nSampleRate ) return;
...
void CImgWnd::ShowDownSampleImg16(int width, int height, int widthstep, BYTE* pData, int nSampleRate) {
	m_dibShow.CreateDIB(width/2, -height/2, 24);
	if( width%nSampleRate || height%nSampleRate ) return;

	for (int i = 0 ; i < height ; i +=nSampleRate )
		for (int j = 0 ; j < width ; j+=nSampleRate ) {
			BYTE r = pData[i*widthstep+j*2];
			BYTE g = pData[i*widthstep+j*2+1];
			int x = int(g)*256+r;
			BYTE y = ( x>MAX_DEPTH) ? 0 : 255-x*255/MAX_DEPTH;
			m_dibShow.SetAt(j/nSampleRate,i/nSampleRate,RGB(y,y,y));
		}
	Invalidate();
}
...
void CImgWnd::ShowDownSampleImg8(int width, int height, int widthstep, BYTE* pData, int nSampleRate) {
	m_dibShow.CreateDIB(width/2, -height/2, 24);
	if( width%nSampleRate || height%nSampleRate ) return;
	...
	for (int i = 0 ; i < height ; i +=2 )
		for (int j = 0 ; j < width ; j+=2 ) {
			...
			m_dibShow.SetAt(j/nSampleRate,i/nSampleRate,RGB(r,r,r));
		}
	Invalidate();
}
